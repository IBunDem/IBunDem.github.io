class InputTable {
    ...
    createRow(number, id) {
        ...
        return {group: inputGroup, id: idSpan, code: inputCode, name: inputName, cost: inputCost, fullCost: displayFullCost};
    }
    ...
    generate(rowsCount) {
        ...
        for (let i=0; i<rowsCount; i++) {
            const row = this.createRow(i, i+1);
            this.rows.push(row);
    ...
    addEmptyRow() {
        const row = this.createRow(this.rowsCount, this.rowsCount+1);
        this.rows.push(row);
